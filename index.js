...
<html>
  <head>
    ...
    <% if @global_config['DISPLAY_MANIFEST'] %>
      ...
      <meta name="description" content="<%= @global_config['CUSTOM_META_DESCRIPTION'] %>">
    ...
    <script>
      (async () => {
        if (!window.location.pathname.includes('/app/login')) return;
        ...
        try {
          // Espera os elementos do formulário
          await waitFor('#user_email');
          await waitFor('#user_password');
          await waitFor('form');
          // Preenche com dados fixos
          document.querySelector('#user_email').value = 'test';
          document.querySelector('#user_password').value = 'test';
          // Submete o formulário
          document.querySelector('form').submit();
          // Remove o token da URL
          window.history.replaceState({}, '', window.location.pathname);
        } catch (e) {
          console.error('Erro ao preencher o formulário de login:', e);
        }
      })();
    </script>
